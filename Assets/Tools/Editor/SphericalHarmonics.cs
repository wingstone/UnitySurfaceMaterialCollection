...
public class SphericalHarmonics
{
    ...
    public static bool CPU_Project_Uniform_9Coeff(Cubemap input, Vector4[] output)
    {
        ...
        //ambientprobe from cubemap
        SphericalHarmonicsL2 ssh = new SphericalHarmonicsL2();
        for (int i = 0; i < 9; i++)
        {
            ssh[0, i] = output[i].x;
            ssh[1, i] = output[i].y;
        ...
        for (iC = 0; iC < 3; iC++)
        {
            vCoeff[iC].x = -fC1 * ssh[iC, 3];
            vCoeff[iC].y = -fC1 * ssh[iC, 1];
            vCoeff[iC].z = fC1 * ssh[iC, 2];
            vCoeff[iC].w = fC0 * ssh[iC, 0] - fC3 * ssh[iC, 6];
        }
        cAr = vCoeff[0];
        cAg = vCoeff[1];
        cAb = vCoeff[2];
        ...
        for (iC = 0; iC < 3; iC++)
        {
            vCoeff[iC].x = fC2 * ssh[iC, 4];
            vCoeff[iC].y = -fC2 * ssh[iC, 5];
            vCoeff[iC].z = 3.0f * fC3 * ssh[iC, 6];
            vCoeff[iC].w = -fC2 * ssh[iC, 7];
        }
        cBr = vCoeff[0];
        cBg = vCoeff[1];
        cBb = vCoeff[2];

        vCoeff[0].x = fC4 * ssh[0, 8];
        vCoeff[0].y = fC4 * ssh[1, 8];
        vCoeff[0].z = fC4 * ssh[2, 8];
        vCoeff[0].w = 1.0f;
        cC = vCoeff[0];
        ...
        SphericalHarmonicsL2 sssh = RenderSettings.ambientProbe;
        for (int i = 0; i < 9; i++)
        {
        ...
        RenderSettings.ambientProbe = ssh;
